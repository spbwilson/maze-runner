...
%  unix(['streamer  -c /dev/video0 -j 90 -s 640x480 -o', filename, '.jpeg > /dev/null 2>&1'])
  unix(['mplayer tv:// -tv driver=v4l:width=640:height=480:device=/dev/video0 -frames 1 -vo jpeg > /dev/null 2>&1']);
  unix(['mv 00000001.jpg ', filename, '.jpg']);

  % read image from capture file
  % use importdata instead of imread to avoid needing 
  % image processing toolkit  
% Im = imread(['Scene', '.jpg'],'jpg'); 
